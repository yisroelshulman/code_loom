 ...
 * Fields
 * - int pathlen
 *      the length of the path to the file
 * - int filenamelen
 ...
 *
 * @returns
 *      true:   if the sourcecode is a valid file type and can be recognized by the compiler
 *      false:  if the sourcecode is not a valid file type
 */
bool init_file(File *file, const char* source);

/*
 * The function compile_file compiles the sourcecode so it can then be run
 ...
 *
 * @returns
 *      true:   if the sourcecode compiled
 *      false:  if the sourcecode did not compile
 */
